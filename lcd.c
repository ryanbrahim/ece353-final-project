 ...
 *      Author: Ryan Almizyed & Sofya Akhetova
 */
...
void HAL_LCD_SpiInit(void)
{
    ...
    EUSCI_B0->CTLW0 =       EUSCI_B_CTLW0_CKPH |            // Phase of 1
                            ...
                            //EUSCI_B_CTLW0__STEM |         // UCxSTE digital output
...
void lcd_set_draw_frame(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
    ...
    HAL_LCD_writeData((uint8_t)(x0>>8));    // Upper 8 bits
    HAL_LCD_writeData((uint8_t)(x0));       // Lower 8 bits
    HAL_LCD_writeData((uint8_t)(x1>>8));    // Upper 8 bits
    HAL_LCD_writeData((uint8_t)(x1));       // Lower 8 bits
    ...
    HAL_LCD_writeData((uint8_t)(y0>>8));    // Upper 8 bits
    HAL_LCD_writeData((uint8_t)(y0));       // Lower 8 bits
    HAL_LCD_writeData((uint8_t)(y1>>8));    // Upper 8 bits
    HAL_LCD_writeData((uint8_t)(y1));       // Lower 8 bits
}
